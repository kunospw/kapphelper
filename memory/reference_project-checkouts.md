...
| Checkout | User | Role | Notes |
|---|---|---|---|
| `/opt/KairosTSApp` on `tsapp` (`cloud2.kairossolutions.co:7723`) | `drini` (Dyah) | Deploy checkout; also holds Dyah's server-side Claude sessions | Commits authored as `Dyah Rini <solutions.dyahr@kairossolutions.co>`. |
