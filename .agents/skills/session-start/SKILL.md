---
name: session-start
description: Orient at the start of a Codex session — pull the latest kapphelper, resolve which project the user is on, read that project's app-repo git state and recent captures, and print a briefing. Use whenever a session begins, when the user says "continue X", or when project context is unclear.
---
