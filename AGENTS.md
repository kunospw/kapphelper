...
Per Ken's rule, this repo lives at **`~/klaudecode/kapphelper/`** — Codex's memory belongs in
...
**Start of session:** run `/session-start` (`.Codex/skills/session-start/`) — git pull → registry
lookup → app-repo state → recent captures → briefing.

**To deploy / restart / tail logs / open DB shell on a live service:** run `/deploy`
(`.Codex/skills/deploy/`) — reads `projects.yaml` for the target's config, runs non-privileged
...
- **Deploy is gated per project.** `projects.yaml` `deploy:` block defines target, host, path,
  ...
  ask — don't hardcode.
- **On the server, Codex has no sudo.** Docker compose in the project folder is the ceiling.
...
- **Docker access varies per host.** On some hosts the user is in the `docker` group and can
  ...
  `sudo` with a password — Codex cannot type that non-interactively. Those projects are
  flagged `deploy.mode: advisory` in `projects.yaml` — Codex prepares the deploy command
...
- **Dyah** runs Codex locally on Windows. Deploys reach the server via SSH.
- **Jesynta** runs Codex on `kconnect01` directly. Deploys run natively (docker compose
  locally on that box).
