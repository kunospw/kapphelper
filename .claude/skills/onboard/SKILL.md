...
- Repo not under `~/klaudecode/` → Ken's rule: Claude memory lives there. Show the move command
...
| Claude Code | `claude --version` | required — they're running it, so present |
...
- Default is **Sonnet 5, effort medium** — the team shares a limited pool (memory
  `model-usage-discipline`). Ask them to run `/status` (or `/usage`) and confirm.
